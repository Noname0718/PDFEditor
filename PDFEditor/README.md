...
## 사용 가능한 기능
- **PDF 뷰어**
  - OpenFileDialog로 파일을 선택하고 PdfiumViewer로 페이지를 렌더링.
  - `다음/이전` 버튼과 페이지 입력 창으로 빠르게 이동.
  - 줌 슬라이더, 직접 입력, `폭에 맞추기` 버튼으로 배율 조정.
- **필기 도구**
  - 펜/형광펜/지우개 세 가지 모드를 지원하며 색상·두께를 통일해서 관리.
  - 지우개 모드는 InkCanvas 잉크뿐 아니라 ShapeToolManager와 연동해 도형도 삭제.
  - PenCursorManager가 굵기와 일치하는 원형 커서를 띄워 실제 입력 크기를 미리 확인.
- **도형 도구**
  - 사각형/원/선/삼각형을 드래그로 생성하고 InkCanvas.Children에 Shape로 배치.
  - 선택 도구로 복수 요소를 드래그·리사이즈·복사/붙여넣기 가능.
  - AreaEraserManager로 텍스트/도형을 같은 도구로 긁어 삭제.
- **텍스트 도구**
  - InkCanvas를 클릭하면 투명 배경과 연한 테두리의 TextBox가 생긴다.
  - 더블클릭/Enter로 편집, ESC나 포커스 이동 시 커밋되며 Undo/Redo 히스토리에 기록.
  - 툴바에서 글꼴 크기, 색상을 즉시 변경하면 선택된 텍스트 박스에도 반영.
- **주석 관리**
  - 현재 PDF의 페이지별 잉크·도형·텍스트를 `.pdfanno`로 저장/불러오기.
  - PDF와 동일 경로·이름의 `.pdfanno`가 있으면 자동 로드.
  - `전체 지우기` 버튼으로 모든 페이지의 주석을 한 번에 초기화.
- **내보내기**
  - 각 페이지를 다시 렌더링해 주석을 합성한 이미지를 만들고 새로운 PDF로 저장.
